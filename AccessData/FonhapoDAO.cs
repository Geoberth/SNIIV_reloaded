...
/// <summary>
/// Descripción breve de FonhapoDAO
/// </summary>
public class FonhapoDAO
{
    private static FonhapoDAO _instancia = null;

    public static FonhapoDAO instancia()
    {
        return _instancia == null ? new FonhapoDAO() : _instancia;
    }
    ...
    {
        string str = "select distinct anio from c_periodo_fonhapo order by anio desc";
    ...
    {
        //string str = "select last_day(concat(anio, '-', mes, '-1')) as fecha from c_periodo_fonhapo order by anio desc , mes desc limit 1";
        string str = "SELECT (date_trunc('month',concat(anio,'-',TO_CHAR(mes,'fm00'),'-', '01')::date)+ interval '1 month' - interval '1 day')::date as fecha from c_periodo_fonhapo where actual";
    ...
    public string[] crearConsulta(string parametro)
    {
        ...
        switch (parametro)
        {
            case "anio":
                ...
                break;
            case "esquema":
                lst[0] = "|l.descripcion as esquema";
                lst[1] = "|id_esquema";
                lst[2] = "|join c_linea_subsidio_fonhapo l on f.id_esquema = l.id";
                break;
            case "estado":
            ...
            case "poblacion_indigena":
                ...
                break;
            case "promotor":
                lst[0] = "|p.descripcion as promotor";
                lst[1] = "|id_promotor";
                lst[2] = "|join c_grupo_promotor p on f.id_promotor = p.id";
                break;
            case "rango_edad":
                ...
                break;
            case "tipo_vivienda":
                lst[0] = "|t.descripcion as tipo_vivienda";
                lst[1] = "|id_tipo_vivienda";
                lst[2] = "|join c_tipo_vivienda_fonhapo t on f.id_tipo_vivienda = t.id";
                break;
            default:
    ...
    public List<FonhapoVO> getCubo(string anios, string clave_estado, string clave_municipio, string dimensiones)
    {
        ...
        List<FonhapoVO> cubo = new List<FonhapoVO>();
        ...
        query.Append(" from cubo_fonhapo_bak ");
        ...
        try
        {
            ...
            cubo = Util.instancia().ConvertDataTable<FonhapoVO>(dt);
        }
        catch (Exception ex) { Util.instancia().setLogError(ex); }
        return cubo;
    }
